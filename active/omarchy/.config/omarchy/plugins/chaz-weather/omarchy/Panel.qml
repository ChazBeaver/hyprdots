...
Panel {
  ...
  readonly property string updatedText: (cacheInfo && cacheInfo.fetched_at) ? timeLabel(cacheInfo.fetched_at, true) : ""
  ...
  function hourLabel(timeString) {
    return timeLabel(timeString, false)
  }

  function timeLabel(timeString, includeMinutes) {
    var value = String(timeString)
    var hour = Number(value.slice(11, 13))
    if (isNaN(hour)) return value
    var suffix = hour >= 12 ? "PM" : "AM"
    var displayHour = hour % 12
    if (displayHour === 0) displayHour = 12
    var minutes = includeMinutes ? ":" + value.slice(14, 16) : ""
